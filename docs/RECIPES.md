...
The generated `TelegramBotApiMethodSchema` covers all 185 Bot API 10.3 methods and 932 documented parameters. It validates required parameters and prevents a DTO scoped to one method from being sent through another method. For configured channels that merge `chat_id` or topic defaults after DTO creation, pass `validateRequiredParameters: false`.
...
The Laravel rate limiter scopes attempts by sanitized bot token hash, method, `business_connection_id`, and `chat_id` when those values are present, so one busy destination does not consume the whole package limit.
...
The generated classes are method-scoped, so the client rejects sending a `SendMessageRequestData` through another Telegram method.

## Webhook Router V2

Use route-level middleware for command or update handlers:
...
Handlers may also be registered with attributes and listed in `telegram-bot.webhook.discover.handlers`.

## Retry, Rate Limit, And API Observability

Enable local retry for retryable Telegram responses and transient transport failures:
...
Enable the Laravel-friendly local rate limiter when outbound bursts should be capped before hitting Telegram:
...
Enable API observability to dispatch `TelegramBotApiRequestRecorded` events with method, status, duration, attempts, and file flag.
The telemetry does not include chat IDs, message text, tokens, or webhook secrets.
...
The package fake supports sequence, typed payload, token leakage, webhook update, and conversation assertions:
