...
- Use `TelegramBot::channel('alerts')->text($text)`, `TelegramBot::to($chatId, token: $token)->photo($photo, caption: $caption)`, or `TelegramBot::document($document, to: $chatId)` for the shortest common outbound sends. Use `AlexItDev91\LaravelTelegramBot\Outbound\TelegramMessage` with `send(TelegramMessage::text($text))` when the message needs reply markup, parse mode, notification flags, content protection, or extra Telegram fields.
